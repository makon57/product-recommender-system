...
export function FakerProducts() {
  ...
  function createRandomProducts() {
    const myProduct: ProductData = {
      ...
      rating: parseFloat(faker.number.float({ min: 0, max: 5, precision: 0.1 }).toFixed(2)),
    };
    return myProduct;
  }
...
export function SingleFakerProducts() {
  function capitalizeFirstLetter(val: string) {
    return String(val).charAt(0).toUpperCase() + String(val).slice(1);
  }
  ...
  function createRandomProducts() {
    const myProduct: ProductData = {
      ...
      price: parseFloat(faker.number.float({ min: 5, max: 20 }).toFixed(2)),
      imageUrl: faker.image.urlLoremFlickr(),
      rating: parseFloat(faker.number.float({ min: 0, max: 5 }).toFixed(2)),
    };
    return myProduct;
  }
